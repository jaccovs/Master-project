    ...
    public void eval() throws OWLOntologyCreationException, DiagnosisException {
        ...
        try {
            ...
            for (String ontologyFile : Configuration.getOntologies()) {
                ...
                boolean continueWithCalculation = true;


                //for (int maxNumberOfDiagnoses : Configuration.getDiagnoseSizes()) {
                Iterator<Integer> it = Configuration.getDiagnoseSizes().iterator();
                for ( ; it.hasNext() && continueWithCalculation ; ) {
                    final Integer maxNumberOfDiagnoses = it.next();
                    ...
                    for (int iteration = 1; iteration <= Configuration.getIterations() && continueWithCalculation; iteration++) {
                        ...
                        // stop with this iteration, diagnosisSize and this ontology if timeout exceeds a threshold
                        continueWithCalculation &= PerfMeasurementManager.getTimer(TIMER_DIAGNOSES_CALCULATION).total() <= Configuration.getMaxTimeoutInNanoSeconds();

                        assertEquals(originalShuffleResult, diagnosisEngine.getSolver().getDiagnosisModel().getPossiblyFaultyFormulas());

                        System.out.println("calculated " + diagnoses.size() + " diagnoses in " + PerfMeasurementManager.getTimer(TIMER_DIAGNOSES_CALCULATION).total() + "ns.");
