...
public class ManchesterOWLSyntaxEditorParser {
    ...
    /*public OWLAxiom parseClassDeclarationAxiom() throws ParserException {
        String kw = consumeToken();


        kw = consumeToken();
        if (isClassName(kw)) {
            OWLClass classClass = getOWLClass(kw);

            // OWLObjectPropertyExpression prop = parseObjectPropertyExpression();
            return dataFactory.getOWLDeclarationAxiom(classClass);
        }
        else {
                    throw createException(false, false, false, true, false, false);
                }
    }*/

    public OWLAxiom parseIndividualClassAssertion() throws ParserException {
        OWLIndividual lhs = parseIndividual();

        String kw = consumeToken();
        if (kw.equalsIgnoreCase(ManchesterOWLSyntax.TYPE.toString())) {

            OWLClassExpression rhs = parseIntersection();
            return dataFactory.getOWLClassAssertionAxiom(rhs, lhs);
        } else if (kw.equalsIgnoreCase(ManchesterOWLSyntax.SAME_AS.toString())) {
            Set<OWLIndividual> individuals = new TreeSet<OWLIndividual>();

            OWLIndividual rhd = parseIndividual();
            individuals.add(lhs);
            individuals.add(rhd);
            return dataFactory.getOWLSameIndividualAxiom(individuals);
        } else if (kw.equalsIgnoreCase(ManchesterOWLSyntax.DIFFERENT_FROM.toString())) {
            Set<OWLIndividual> individuals = new TreeSet<OWLIndividual>();
            ...
            if (kw.equalsIgnoreCase(ManchesterOWLSyntax.NOT.toString())) {
                kw = consumeToken();
                negative = true;
            }

            if (isDataPropertyName(kw)) {
                OWLDataProperty p = getOWLDataProperty(kw);
                OWLLiteral con = parseLiteral();
                if (!negative) {
                    return dataFactory.getOWLDataPropertyAssertionAxiom(p, lhs, con);
                } else {
                    return dataFactory.getOWLNegativeDataPropertyAssertionAxiom(p, lhs, con);
                }
            } else if (isObjectPropertyName(kw)) {
                OWLObjectPropertyExpression p = getOWLObjectProperty(kw);
                kw = consumeToken();
                if (isIndividualName(kw)) {
                    OWLIndividual obj = getOWLIndividual(kw);
                    if (!negative) {
                        return dataFactory.getOWLObjectPropertyAssertionAxiom(p, lhs, obj);
                    } else {
                        return dataFactory.getOWLNegativeObjectPropertyAssertionAxiom(p, lhs, obj);
                    }
                } else {
                    throw createException(false, false, false, true, false, false);
                }
        ...
        throw createException(false, true, true, false, false, false, ManchesterOWLSyntax.TYPE.toString(), ManchesterOWLSyntax.SAME_AS.toString(), ManchesterOWLSyntax.NOT.toString(), ManchesterOWLSyntax.DIFFERENT_FROM.toString());
    }
    ...
    public static final String INVERSE_OF = ManchesterOWLSyntax.INVERSE_OF.toString();

    public static final String RULE = ManchesterOWLSyntax.RULE.toString();
    ...
    public ManchesterOWLSyntaxEditorParser(OWLOntologyLoaderConfiguration configuration, OWLDataFactory dataFactory, String s) {
        ...
        potentialKeywords = new TreeSet<String>();

        classNames = new TreeSet<String>();
        objectPropertyNames = new TreeSet<String>();
        dataPropertyNames = new TreeSet<String>();
        individualNames = new TreeSet<String>();
        dataTypeNames = new TreeSet<String>();
        annotationPropertyNames = new TreeSet<String>();
...
//            String[] res = u.split(iri.toString(), null);
        	String[] res = {iri.getNamespace(),iri.getShortForm()};
...
//            return tokenIndex < tokens.size() ? tokens.get(tokenIndex) : tokens.get(tokens.size() - 1);
    ...
    public OWLClassExpression parseObjectRestriction() throws ParserException {
        ...
        if (kw.equalsIgnoreCase(SOME)) {
            ...
            String tok = peekToken();
            Set<OWLClassExpression> descs = new TreeSet<OWLClassExpression>();
            if (!tok.equals("[")) {
...
//            	return dataFactory.getOWLDataSomeValuesFrom(prop, rng)
    ...
    private OWLDataRange parseDataOneOf() throws ParserException {
        consumeToken();
        Set<OWLLiteral> cons = new TreeSet<OWLLiteral>();
        String sep = ",";
        while (sep.equals(",")) {
            OWLLiteral con = parseConstant();
            cons.add(con);
    ...
    public Set<OntologyAxiomPair> parseFrames() throws ParserException {
        Set<OntologyAxiomPair> axioms = new TreeSet<OntologyAxiomPair>();
    ...
    public Set<OntologyAxiomPair> parseNaryEquivalentClasses() throws ParserException {
        ...
        Set<OntologyAxiomPair> pairs = new TreeSet<OntologyAxiomPair>();
    ...
    public Set<OntologyAxiomPair> parseNaryEquivalentProperties() throws ParserException {
        ...
        if (properties.iterator().next().isObjectPropertyExpression()) {
            Set<OWLObjectPropertyExpression> ope = new TreeSet<OWLObjectPropertyExpression>();
            ...
            Set<OWLDataPropertyExpression> dpe = new TreeSet<OWLDataPropertyExpression>();
            for (OWLPropertyExpression pe : properties) {
...
//        }
    ...
    private Set<OWLOntology> parseOntologyList() throws ParserException {
        ...
        consumeToken("in");
        String sep = ",";
        Set<OWLOntology> onts = new TreeSet<OWLOntology>();
        while (sep.equals(",")) {
            String tok = consumeToken();
    ...
    public SWRLDArgument parseDObject() throws ParserException {
        ...
        if (s.equals("?")) {
            ...
            try {
                ...
                Set<String> kw = new TreeSet<String>(e.getExpectedKeywords());
                kw.add("?");
...
//    }
    ...
    public Map<OWLClassExpression, Set<Set<OWLAnnotation>>> parseAnnotatedClassExpressionList() throws ParserException {
        ...
        while (sep.equals(",")) {
            ...
            if (annosSet == null) {
                annosSet = new TreeSet<Set<OWLAnnotation>>();
    ...
    public Set<OWLPropertyExpression> parsePropertyList() throws ParserException {
        Set<OWLPropertyExpression> props = new TreeSet<OWLPropertyExpression>();
        String sep = ",";
        while (sep.equals(",")) {
            OWLPropertyExpression prop = parsePropertyExpression();
            props.add(prop);
    ...
    public Set<IRI> parseNameList() throws ParserException {
        ...
        while (sep.equals(",")) {
            ...
            if (isClassName(token)) {
                ...
                iris.add(getOntology(token).getOntologyID().getOntologyIRI().get());
                consumeToken();
    ...
    @Deprecated
    public ManchesterSyntaxDocumentFormat parseOntology(OWLOntologyManager manager, OWLOntology ont) throws ParserException, UnloadableImportException {
        return parseOntology(ont);
    }

    public ManchesterSyntaxDocumentFormat parseOntology(OWLOntology ont) throws ParserException, UnloadableImportException {
        Set<OntologyAxiomPair> axioms = new TreeSet<OntologyAxiomPair>();
        OWLOntologyID ontologyID = new OWLOntologyID();
        Set<AddImport> imports = new TreeSet<AddImport>();
        Set<AddOntologyAnnotation> ontologyAnnotations = new TreeSet<AddOntologyAnnotation>();
    ...
    public ManchesterOWLSyntaxOntologyHeader parseOntologyHeader(boolean toEOF) throws ParserException {
        ...
        Set<OWLImportsDeclaration> imports = new TreeSet<OWLImportsDeclaration>();
        while (true) {
            String section = peekToken();
            if (section.equals(IMPORT)) {
                ...
                if (tok.startsWith("<")) {
                    ...
                    OWLOntology ont = getOntology(tok);
                    if (ont != null) {
                        importedIRI = ont.getOntologyID().getOntologyIRI().get();
                    }
                } else {
                    consumeToken();
    ...
    protected ParserException createException(boolean ontologyNameExpected) throws ParserException {
        ...
        return new ParserException(getTokenSequence(), lastToken.getPos(), lastToken.getRow(), lastToken.getCol(), ontologyNameExpected, false, false, false, false, false, false, false, null);
    }


    protected void throwOntologyNameOrURIExpectedException() throws ParserException {
        ManchesterOWLSyntaxTokenizer.Token lastToken = getLastToken();
        Set<String> expectedKeywords = new LinkedHashSet<String>();
        expectedKeywords.add("<$ONTOLOGYYURI$>");
        throw new ParserException(getTokenSequence(), lastToken.getPos(), lastToken.getRow(), lastToken.getCol(), true, false, false, false, false, false, false, false, expectedKeywords);
    }


    protected ParserException createException(String... keywords) throws ParserException {
        Set<String> theKeywords = new TreeSet<String>();
    ...
    public OWLAxiom parseAxiom() throws ParserException {
        ...
        if (isClassName(token)) {
            ...
            return parseIndividualClassAssertion();
            ...
            /*} else if (token.equalsIgnoreCase(CLASS)) {
                return parseClassDeclarationAxiom();
            } else if (token.equalsIgnoreCase(DATATYPE)) {
                return null;
                // return parseDatatypeDeclarationAxiom();
            } else if (token.equalsIgnoreCase(OBJECT_PROPERTY)) {
                return null;
                // return parseObjectPropertyDeclarationAxiom();
            } else if (token.equalsIgnoreCase(DATA_PROPERTY)) {
                return null;
                // return parseDataPropertyDeclarationAxiom();
            } else if (token.equalsIgnoreCase(INDIVIDUAL)) {
                return null;
                // return parseIndividualDeclarationAxiom(); */
        } else {
            ...
            // CLASS,DATATYPE,DATA_PROPERTY,OBJECT_PROPERTY,  INDIVIDUAL);
        }
...
//    }
    ...
    private <F, O> Set<OntologyAxiomPair> parseAnnotatedListItems(F subject, AnnotatedListItemParser<F, O> itemParser, Set<OWLOntology> ontologies) throws ParserException {
        Set<OntologyAxiomPair> result = new TreeSet<OntologyAxiomPair>();
        String sep = ",";
        while (sep.equals(",")) {
            String tok = peekToken();
            Set<OWLAnnotation> annotations;
    ...
    private class ClassHasKeyListItemParser extends AnnotatedPropertyListListItemParser<OWLClass> {
        public OWLAxiom createAxiom(OWLClass subject, Set<OWLPropertyExpression> object, Set<OWLAnnotation> annotations) {
            Set<OWLPropertyExpression> propertyExprSet = new HashSet<OWLPropertyExpression>();

            for (OWLPropertyExpression expr : object)
                propertyExprSet.add(expr);


            return dataFactory.getOWLHasKeyAxiom(subject, propertyExprSet, annotations);
        }
