 ...
 * @author dietmar
 */
public class ParallelHSDagEngine<T> extends AbstractHSDagEngine<T> {
    ...
    public ParallelHSDagEngine(DiagnosisModel<T> sessionData, int threadPoolSize) {
        super(sessionData);
     ...
     * @return
     */
    public static <T> int calculateEdgeLevelCount(List<Node<T>> nodes) {
        int result = 0;
        //if the node has a reduced nodeLabel set (resulting from duplicate path check) then use this size instead.
        //otherwise the barrier will have the incorrect number of threads to wait for and will wait indefinitely.
        for (Node<T> node : nodes) {
            result += (node.reducedConflict == null) ? node.nodeLabel.size() : node.reducedConflict.size();
        }
        return result;
    }
     ...
     */
    public static <T> void checkForDuplicatePaths(List<Node<T>> nodesToExpand) {
        ...
        List<Node<T>> nodesToExpandCopy = new ArrayList<Node<T>>(nodesToExpand);
        ...
        // DAG, update the nodeLabel label of the node and remove the redundant
        // element. need to make sure that we do not change the cached nodeLabel itself
        for (Node<T> node : nodesToExpandCopy) {
            List<T> conflictAtNode = node.nodeLabel;
            if (conflictAtNode == null) {
//				System.out.println("No nodeLabel at node?");
                return;
            }
        ...
        if (rootNode == null) {
            ...
            try {
                ...
                ConflictCheckingResult<T> checkingResult = qx.checkExamples(
                        this.sessionData.getDiagnosisModel().getPositiveExamples(),
                        new ArrayList<T>(),
                        true);
...
//						allConstructedNodes.addItem(rootNode);
                        ...
                        List<Node<T>> nodesToExpand = new ArrayList<Node<T>>();
...
//						for (List<T> nodeLabel: conflicts)
//						{
//							System.out.println(Utilities.printConstraintList(nodeLabel, model));
//						}
...
//		long end = 0;
        ...
        SharedCollection<Node<T>> newNodesToExpand = new SharedCollection<Node<T>>();
...
//			System.out.println("NODES TO EXPAND: " + nodesToExpand.size() + " at level " + currentLevel);
            ...
            Node<T> node = nodesToExpand.remove(0);
//			System.out.println("Working on node with label: " + Utilities.printConstraintList(node.pathLabels, this.model));
            ...
            if (node.nodeLevel < this.sessionData.getConfiguration().searchDepth || this.sessionData.getConfiguration().searchDepth == -1) {
...
//				System.out.println("Reached level, nodeLevel " + node.nodeLevel + ", searchdepth: " + this.sessionData.getConfiguration().searchDepth);
            }
        }
//		System.out.println("Have started: " + totalEdgeWorkersStarted + " edgeworkers, MY LIST IS EMPTY, waiting for barrier: " + currentLevel  + ", cdl: " + barrier.getCount());
        try {
            ...
            nodesToExpand = new ArrayList<Node<T>>(newNodesToExpand.getCollection());
...
//			e.printStackTrace();
        ...
        if (!nodesToExpand.isEmpty()) {
            ...
            if ((this.searchDepth == -1 || this.currentLevel < this.searchDepth) &&
                ...
                if (currentLevel < this.sessionData.getConfiguration().searchDepth || this.sessionData.getConfiguration().searchDepth == -1) {
                    // TEST TS ************
