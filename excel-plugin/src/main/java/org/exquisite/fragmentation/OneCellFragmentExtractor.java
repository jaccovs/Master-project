...
public class OneCellFragmentExtractor implements IFragmentExtractor {
    ...
    private boolean isCopyEquivalent(Fragment f1, Fragment f2) {
        ...
        if ((f1.getInputs().size() != f2.getInputs().size()) || (f1.getInterims().size() != f2.getInterims()
                .size()) || (f1.getOutputs().size() != f2.getOutputs().size())) {
            return false;
        }

        for (int i = 0; i < f1.getInterims().size(); i++) {
            if (!appXML.getFormulasR1C1().get(f1.getInterims().get(i))
                    .equals(appXML.getFormulasR1C1().get(f2.getInterims().get(i)))) {
                return false;
            }
        }

        for (int i = 0; i < f1.getOutputs().size(); i++) {
            if (!appXML.getFormulasR1C1().get(f1.getOutputs().get(i))
                    .equals(appXML.getFormulasR1C1().get(f2.getOutputs().get(i)))) {
                return false;
            }
        }
