...
public class ValueInteractivityTest {
    ...
    List<Diagnosis<Constraint>> lastCalculatedDiagnoses;
    DiagnosisModel<Constraint> lastCalculatedModel;
...
//			test.runScenario("formula_query_example.xml", "formula_query_example_correct.xml");
    ...
    private void removeCells(List<String> interimsToCheck, Dictionary<String, String> cellsToRemove) {
        for (int i = 0; i < interimsToCheck.size(); i++) {
            ...
            if (cellsToRemove.get(cell) != null) {
                System.out
                        .println("Removed " + cell + " from interimsToCheck because it is marked as correct / faulty.");
                interimsToCheck.remove(i);
                i--;
            }
        }
	...
	private int findCellWithMinimalDiagnoses(String mutatedXMLFilename, ExquisiteAppXML mutatedXML, ExquisiteAppXML correctXML, int lastMinDiagnoses) {
        ...
        DiagnosisModel<Constraint> model = null;
        if (cellDeterminationHeuristic == CellDeterminationHeuristic.FirstDiagnosis) {
     ...
     * @return
     */
    private String chooseCellForInteraction(Dictionary<String, Integer> cellsAndDiagnoses, List<Diagnosis<Constraint>>
            diagnoses, Dictionary<String, String> faultyValues, DiagnosisModel<Constraint> model) {
        String chosenCell = "";
     ...
     * @return
     */
    private int calculateDiagnoses(String mutatedXMLFilename, String additionalCorrectCell,
                                   String additionalCorrectCellsValue) {
        ExquisiteAppXML newXml = ExquisiteAppXML.parseToAppXML(inputFileDirectory + mutatedXMLFilename);

        Dictionary<String, String> newXmlCorrectValues = newXml.getTestCases().elements().nextElement()
                .getCorrectValues();
     ...
     * @return
     */
    private int calculateDiagnosesCount(List<Diagnosis<Constraint>> diagnoses, DiagnosisModel<Constraint> oldModel,
                                        String additionalCorrectCell, String additionalCorrectCellsValue) {
...
//		DiagnosisModel model = diagnosisEngine.sessionData.diagnosisModel;
        ...
        try {
            ...
            for (int i = 0; i < diagnoses.size(); i++) {
                ...
                DiagnosisModel<Constraint> newModel = new DiagnosisModel<>(startModel);
