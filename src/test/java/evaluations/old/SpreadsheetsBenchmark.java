...
//					loggingResult+="(Diag # " + j + ": " + Utilities.printConstraintList(diagnoses.get(j).getElements(), sessionData.diagnosisModel) + ") ";
				...
				System.out.println("Number of known conflicts: " + ((AbstractHSDagBuilder) diagnosisEngine).knownConflicts.getCollection().size());
				List<List<Constraint>> theknown = ((AbstractHSDagBuilder) diagnosisEngine).knownConflicts.getCollection();
				for (List<Constraint> conflict : theknown) {
					System.out.println(Utilities.printConstraintListOrderedByName(conflict, sessionData.diagnosisModel));
				}
