 ...
 * @author Thomas
 */
public class InvQXDiagnosisEngine implements IDiagnosisEngine {
	 ...
	 * @param sessionData
	 */
	public InvQXDiagnosisEngine(ExquisiteSession sessionData){
	...
	public List<Diagnosis> calculateDiagnoses() throws DiagnosisException {
		InverseQuickXplain invQX = new InverseQuickXplain(getSessionData(), this);
		
		List<List<Constraint>> conflicts = invQX.findConflicts();
		List<Diagnosis> diagnoses = new ArrayList<Diagnosis>();
		
		for (List<Constraint> conflict: conflicts) {
			Diagnosis diag = new Diagnosis(conflict, model);
			diagnoses.add(diag);
		}
