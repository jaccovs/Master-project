...
public class ExquisiteSolverThread extends Thread {
	...
	public void run () {
        ...
        {     
        	// Default choco set
        	workers.add(new SolverThread(cGroup, new CPSolver(), model, SearchStrategies.Default, exquisiteParallelSearch));
			// ImpactBasedBranching
        	workers.add(new SolverThread(cGroup, new CPSolver(), model, SearchStrategies.ImpactBasedBranching, exquisiteParallelSearch));
        	// AssignOrForbidIntVarVal        	
        	workers.add(new SolverThread(cGroup, new CPSolver(), model, SearchStrategies.AssignOrForbidIntVarVal_DomDDegBin, exquisiteParallelSearch));
...
//        	System.out.println("--------- STRATEGY TO USE: " + this.exquisiteParallelSearch.sessionData.config.searchStrategy);
        	SolverThread worker = new SolverThread(cGroup, new CPSolver(), model, this.exquisiteParallelSearch.sessionData.config.searchStrategy, exquisiteParallelSearch);
        	workers.add(worker);
        }
