 ...
 *
 */
public class MergeXplain extends QuickXPlain {
	 ...
	 * @param sessionData
	 */
	public MergeXplain(ExquisiteSession sessionData, IDiagnosisEngine diagnosisEngine) {
...
//		    System.out.println("Done with QX - release the listener");
	 ...
	 */
	@Override
	public void findConflictsParallel(ConflictCheckingResult result,
			SharedCollection<List<Constraint>> knownConflicts)
...
//		Debug.syncMsg("MXP: Finished.");
	...
	private MergeXplainResult mergeXplain(
			...
			List<Constraint> correctStatements) throws DomainSizeException {
...
//		System.out.println("Found " + result.Conflicts.size() + " conflict(s) with splitting technique.");
	...
	private MergeXplainResult mergeXplainParallel(
			List<Constraint> possiblyFaultyStatements,
			List<Constraint> correctStatements, ConflictCheckingResult endResult,
			SharedCollection<List<Constraint>> knownConflicts) throws DomainSizeException {
...
//			Debug.syncMsg("MergeXplain stopped.");
		...
		// If we have only one possibly faulty statement, it has to be a conflict
		if (possiblyFaultyStatements.size() == 1) {
			List<Constraint> conflict = new ArrayList<Constraint>(possiblyFaultyStatements);
			result.Conflicts.add(conflict);
...
//				}
	...
	private List<Constraint> findSingleConflict(
			...
			List<Constraint> c) throws DomainSizeException, InterruptedException {
