 ...
 *
 */
public class Choco2ToChoco3Solver implements ISolver {
	...
	// The variable map
	public Map<Variable, IntVar> theVariables = new HashMap<Variable, IntVar>();
	...
	public solver.constraints.Constraint buildConstraint(Constraint constraint) throws Choco2ToChoco3Exception {
		...
		case AND:
			if (constraint instanceof MetaConstraint<?>) {
				solver.constraints.Constraint[] c3andcons = buildConstraintsOfMetaConstraint((MetaConstraint<?>)constraint);
...
//				c3Constraint = LogicalConstraintFactory.or(c3orcons);
			...
			break;
		case NOT:
			solver.constraints.Constraint[] c3notcons = buildConstraintsOfMetaConstraint((MetaConstraint<?>)constraint);
		...
		case REIFIEDNAND:
			...
			c3Constraint = LogicalConstraintFactory.reification(c3var, LogicalConstraintFactory.not(LogicalConstraintFactory.and(c3vars)));
			break;
		case REIFIEDOR:
			if (constraint.getNbVars() < 3) {
				throw new WrongVariableCountException(constraint.pretty() + " Vars: " + constraint.getNbVars());
		...
		case REIFIEDNOR:
			...
			c3Constraint = LogicalConstraintFactory.reification(c3var, LogicalConstraintFactory.not(LogicalConstraintFactory.or(c3vars)));
			break;
		case REIFIEDXOR:
			if (constraint.getNbVars() < 3) {
				throw new WrongVariableCountException(constraint.pretty() + " Vars: " + constraint.getNbVars());
			...
			c3Constraint = LogicalConstraintFactory.reification(c3var, IntConstraintFactory.sum(c3vars, VariableFactory.fixed(1, solver)));
			break;
		default:
	 ...
	 * @throws Choco2ToChoco3Exception
	 */
	private solver.constraints.Constraint[] buildConstraintsOfMetaConstraint(MetaConstraint<?> constraint) throws Choco2ToChoco3Exception {
	 ...
	 * @throws Choco2ToChoco3Exception
	 */
	private solver.constraints.Constraint buildArithm(Variable var1, String op, Variable var2) throws Choco2ToChoco3Exception {
...
//				}
			...
			case IFTHENELSE:
				...
				if (cs.length != 1) {
					throw new OperatorTypeNotKnownException(expVar.getOperator().name + " Constraints: " + cs.length);
				}
...
//			solver.post(IntConstraintFactory.arithm(c3expVar, "=", c3v1, "-", c3v2));
		...
		case CONSTANT_INTEGER:
			solver.variables.IntVar intvar = VariableFactory.fixed(((IntegerConstantVariable)var).getValue(), solver);
			return intvar;
		default:
	...
	private void createVariables() {
		...
		for (Variable v : variables) {
			...
			solver.variables.IntVar intvar = VariableFactory.bounded(v.getName(), integervar.getLowB(), integervar.getUppB(), solver);
