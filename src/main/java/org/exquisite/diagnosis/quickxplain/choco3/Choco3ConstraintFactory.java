...
//			System.out.println("Have to translate constraint: " + formulaInfo.formula);
            ...
            if (formulaInfo.tree != null) {
                c = buildAndPostChoco3Constraint(formulaInfo.cellName, formulaInfo.cellName, formulaInfo.operator,
                        formulaInfo.tree, ConstraintsFactory.WORKSHEET_PREFIX, formulaInfo.formula);
            } else {
...
//					System.out.println("Creating new variable for target cell " + formulaInfo.cellName);
                    cellVariable = VariableFactory
                            .bounded(formulaInfo.cellName, cellVariable.getLB(), cellVariable.getUB(), c3solver.solver);
                    c3solver.variablesMap.put(formulaInfo.cellName, cellVariable);
                }
                // Post the constraint
                c = IntConstraintFactory
                        .arithm(cellVariable, formulaInfo.operator, Integer.parseInt(formulaInfo.formula));
...
//		return Choco.eq(variables.get(cellReference), (IntegerExpressionVariable) expression));
        ...
        if (cellVariable == null) {
            ...
            cellVariable = VariableFactory
                    .bounded(cellReference, cellVariable.getLB(), cellVariable.getUB(), c3solver.solver);
        ...
        switch (tree.toString().toUpperCase()) {
            ...
            default:
                if (
                    ...
                    return IntConstraintFactory
                            .arithm(buildExpression(cellReference, tree.getChild(0), worksheetReferencePrefix),
                                    symbol,
                                    buildExpression(cellReference, tree.getChild(1), worksheetReferencePrefix));
                }
...
//            		System.out.println("Creating an if");
                    Constraint[] theConstraints = buildConstraintsFromChildren(cellReference, tree,
