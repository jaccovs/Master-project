...
public class SharedDAGNodeQueue<T> extends SharedCollection<T> {
	 ...
	 * @param node
	 */
	public void processNewDAGNodeFullParallel(DAGNode newNode,
			AbstractHSDagBuilder dagBuilder, DiagnosisModel model)
			throws Exception {
			// Need to some work
...
//			System.out.println("New diag: " + Utilities.printConstraintList(newNode.tests.diagnosis, model));
			...
			boolean closedBecauseOfSuperset = NodeUtilities.applyNodeClosingRulesEQ(newNode,dagBuilder.diagnosisNodes.getCollection());
...
//				dagBuilder.diagnoses.add(new Diagnosis(newNode.tests.diagnosis, model));
				...
				for (DAGNode node : (List<DAGNode>) this.getCollection()) {
