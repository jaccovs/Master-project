 ...
 * 
 */
public class FullParallelNodeExpander extends NodeExpander {
	 ...
	 * @param dagBuilder
	 */
	public FullParallelNodeExpander(AbstractHSDagBuilder dagBuilder,
	 ...
	 */
	@Override
	public void expandNode(DAGNode targetNode, Constraint edgeLabel)
			throws DomainSizeException {
...
//		System.out.println("T: " + (System.nanoTime() - ((FullParallelHSDagBuilder)dagBuilder).start) + " FullParallel threads: " + FullParallelHSDagBuilder.runningThreads + " MergeXplain threads: " + QuickXPlain.runningThreads);
		...
		diagnosesCopy = new ArrayList<DAGNode>(
		...
		if (!isSuperset) {
			...
			DAGNode newNode = NodeUtilities.checkForExistingNode(newPathLabels,
	...
	//								Debug.syncMsg("Main thread sleeping.");
									...
									if (!checkingResult.conflictFound())
									{
										...
										if (newConflictsCount > lastConflictsCount) {
											List<List<Constraint>> conflicts = dagBuilder.knownConflicts.getCopy().subList(lastConflictsCount, newConflictsCount - 1);
...
//										this.dagBuilder.nodesToExpand.clear();
				...
				// ---------------------------------------------------------------
				// Here we are.
				SharedDAGNodeQueue<DAGNode> thenodes = (SharedDAGNodeQueue<DAGNode>) this.newNodesToExpand;
				try {
