...
public class PRDFSNodeExpander extends Thread {
	...
	public ParallelRandomDFSEngine engine;
...
//	Constraint startConstraint;
	 ...
	 * @param latch
	 */
	public PRDFSNodeExpander(ParallelRandomDFSEngine engine, NodeSelector nodeSelector) {
		super();
		this.setDaemon(true);
		this.engine = engine;
//		this.startNode = root;
...
//		System.out.println("more to do: " + engine.nodeSelector.hasMoreConstraints(node));
		...
		while (!Thread.currentThread().isInterrupted() && !(engine.sessionData.config.maxDiagnoses > 0 && engine.knownDiags.getCollection().size() >= engine.sessionData.config.maxDiagnoses)) {
			NodeWithConstraint nodeWithConstraint;
...
//				System.out.println("Something to expand");
				ExtendedDAGNode node = nodeWithConstraint.node;
				Constraint c = nodeWithConstraint.constraint;
