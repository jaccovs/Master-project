 ...
 * @author Dietmar
 */
public class HSDagBuilder<T> extends AbstractHSDagBuilder<T> {
            ...
            try {
                ...
                if (knownConflicts.size() > 0) {
                    ...
                    if (!QuickXPlain.CONTINUE_AFTER_FIRST_CONFLICT) {
                        ...
                        checkingResult = qxplain
                                .checkExamples(model.getPositiveExamples(), new ArrayList<>(), true);
                    } else {
                        synchronized (QuickXPlain.ContinuingSync) {
                            checkingResult = new ConflictCheckingResult<>();
                            //						Debug.syncMsg("Start first checkExamplesParallel()");
                            qxplain.checkExamplesParallel(model.getPositiveExamples(), new ArrayList<>(),
                                    true, checkingResult, knownConflicts);
...
//						knownConflicts.addAll(checkingResult.conflicts);
                        ...
                        List<DAGNode<T>> nodes = new ArrayList<DAGNode<T>>();
                        nodes.add(rootNode);
                        conflictNodeLookup.put(rootNode.conflict, nodes);
                        nodesToExpand = new ArrayList<DAGNode<T>>();
...
//		System.out.println("All constructed nodes: " + this.allConstructedNodes.getCollection().size());
        ...
        while (!nodesToExpand.isEmpty()) {
            ...
            DAGNode<T> targetNode = nodesToExpand.remove(0);
...
//					}
                    ...
                    //prune conflict set.
                    boolean isPruned = NodeUtilities
                            .applyPruningRules(targetNode.conflict, knownConflicts.getCollection(), conflictNodeLookup);
//					if (isPruned) {
...
//					}
                    if (!isPruned) {
                        ...
                        for (T c : targetNode.conflict) {
                            NodeExpander<T> expander = new NodeExpander<>(this);
