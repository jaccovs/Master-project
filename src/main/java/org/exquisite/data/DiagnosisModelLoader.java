...
{		
	 ...
	 */
	public DiagnosisModelLoader(ExquisiteSession sessionData, VariablesFactory varFactory, ConstraintsFactory conFactory)
	{
	...
	public void loadDiagnosisModelFromXML(){
		...
		List<IntegerVariable> varsWithGlobalValueBounds = varFactory.makeVariablesWithGlobalValueBounds(appXML.getValueBounds(), appXML.getCellsInRange());
		...
		for(Constraint correctStatement : correctFormulae){
			this.sessionData.diagnosisModel.
				addCorrectConstraint(correctStatement, this.sessionData.diagnosisModel.getConstraintName(correctStatement));
		}
		
		//Add global value bound constraints.
		for(Constraint constraint : conFactory.getPositiveExample().constraints){
			this.sessionData.diagnosisModel.addCorrectConstraint(constraint, conFactory.getPositiveExample().constraintNames.get(constraint));
		}	
		...
		List<Example> examples = conFactory.makeExamplesFromTestCases(appXML.getTestCases(), appXML.getCellsInRange(), varFactory.getVariablesMap(), appXML.getInputs());
		...
		while (fragments.hasMoreElements()) {
			Fragment fragment = (Fragment) fragments.nextElement();
			if (fragment.getTestCases() != null && fragment.getTestCases().size() > 0) {
				examples.addAll(conFactory.makeExamplesFromTestCases(fragment.getTestCases(), appXML.getCellsInRange(), varFactory.getVariablesMap(), fragment.getInputs()));
	 ...
	 * @param formulaParser
	 */
	private List<Constraint> findCorrectStatementsInParser(Dictionary<String, String> correctFormulae, FormulaParser formulaParser){			
