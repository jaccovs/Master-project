...
public class ConstraintsFactory {
	 ...
	 */
	public ConstraintsFactory(Example globalNegativeExample, Example globalPositiveExample, ExquisiteSession sessionData)
	{
	...
	public ConstraintsFactory(ExquisiteSession sessionData){
		this();
		this.sessionData = sessionData;
	}
	...
	public Example getNegativeExample()
	{
		return this.negativeExample;
	}
	
	public Example getPositiveExample()
	{
...
//			System.out.println("Formula: " + formula);
			...
			formulaParser.buildChocoStatements(id, cellReference, formulaParser.FormulaTree, variablesMap, WORKSHEET_PREFIX);
	 ...
	 */
	public Dictionary<String, Constraint> makeGlobalValueBoundsConstraints(Dictionary<String, ExquisiteValueBound> valueBounds, Dictionary<String, List<String>> cellsInRange, Dictionary<String, IntegerExpressionVariable> variablesMap )
	{		
...
//			int max = (int)valueBound.getUpper();			
			...
			if (Utilities.isCellRangeReference(cellReference))
			{
				...
				while(cellsIterator.hasNext())
				{
				  ...
				  Constraint constraint = makeGlobalValueBoundsConstraint(cellInRangeReference, valueBound, variablesMap);
				  String constraintName = "globalValueBoundConstraint (" + valueBound.getLower() + " < " + cellInRangeReference + " < " + valueBound.getUpper() + ")"; 
			...
			else
			{
				...
				String constraintName = "globalValueBoundConstraint (" + valueBound.getLower() + " < " + cellReference + " < " + valueBound.getUpper() + ")";
				result.put(constraintName, constraint);
			}			
		}	
	...
	private  Dictionary<String, Constraint> makeValueBoundsConstraints(TestCase targetTestCase,  
			Dictionary<String, IntegerExpressionVariable> variablesMap, Map<Constraint, FormulaInfo> formulaInfos)
	{
		Dictionary<String, Constraint> result = new Hashtable<String, Constraint>();		
...
//				// DJ: Can there be no cells in some range?
	 ...
	 */
	public List<Example> makeExamplesFromTestCases(Dictionary<String, TestCase> testCases, Dictionary<String, List<String>> cellsInRange, Dictionary<String, IntegerExpressionVariable> variablesMap, List<String> inputCells)
	{		
	 ...
	 */
	public Example makeExamplesFromTestCase(	TestCase targetTestCase, 
												...
												List<String> inputCells)
	{
		...
		Dictionary<String, Constraint> testCaseValueBounds = makeValueBoundsConstraints(targetTestCase, variablesMap, result.choco3FormulaInfos);
		...
		if (PRUNE_IRRELEVANT_CELLS)
		{
			...
			{			 
				...
				if(!allParents.contains(cellReference))
				{
					result.irrelevantConstraints.put(cellReference, Utilities.getKeyByValue(this.sessionData.diagnosisModel.getConstraintNames(), cellReference));				
				}			
			}
		}
		...
		Constraint negativeConstraint = makeNegativeExampleConstraints(targetTestCase.getValues(), targetTestCase.getFaultyValues(), variablesMap, inputCells);
	...
	private Constraint makeNegativeExampleConstraints(Dictionary<String, String> testCaseValues, 
		...
		{			
			...
			if (cellValue == null)
			{
				Debug.msg("    Some values needed to construct a negative example are missing, therefore cancel negative example construction for this test case.");
				return null;			
			}
	 ...
	 */
	public Constraint makeGlobalValueBoundsConstraint(String cellReference, ExquisiteValueBound bounds,  Dictionary<String, IntegerExpressionVariable> variablesMap) 
	{	
		IntegerExpressionVariable cell = variablesMap.get(cellReference);
		List<Constraint> constraints = new ArrayList<Constraint>();
		int min = (int)bounds.getLower();
