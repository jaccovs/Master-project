...
public class VariablesFactory {
     ...
     */
    public List<IntegerVariable> makeVariablesWithGlobalValueBounds(Dictionary<String, ExquisiteValueBound> valueBounds,
            ...
            if (Utilities.isCellRangeReference(cellReference)) {
                ...
                for (String cellName : cells) {
                    ...
                    if (!variableAlreadyExists(cellName)) {
                        result.add(makeIntegerVariable(cellName, (int) valueBound.getLower(),
                                (int) valueBound.getUpper()));
                    }
                }
            } else {
                //if it is just a single variable to make
                if (!variableAlreadyExists(cellReference)) {
                    result.add(makeIntegerVariable(cellReference, (int) valueBound.getLower(),
                            (int) valueBound.getUpper()));
                }
            }
        }
     ...
     * @param model
     * @param min
     * @param max   containing formulae as needed when processing the test case data.
