...
import org.exquisite.data.StringPreprocessor;
...
import org.exquisite.datamodel.ExquisiteMessageItem;
import org.exquisite.datamodel.ExquisiteSession;
import org.exquisite.datamodel.Fragment;
import org.exquisite.datamodel.serialisation.CellNamesParser;
import org.exquisite.datamodel.serialisation.ExquisiteCellNames;
...
import org.exquisite.datamodel.serialisation.ExquisiteFragmentation;
...
public class ServerProtocol implements INotifyingThreadListener {
	...
	{	
		if(this.sessionData.config.showServerDebugMessages){
			System.out.println("processClientInput, client message = (" + message + ") server state at start = " + stateLookup[this.state]);
		}
			...
			case STATE_IDLE:
				...
				if (ClientMessages.REQUEST_FORMULAS_TO_QUERY.equalsIgnoreCase(message)) {
					if (diagnoses == null) {
						...
						FormulaQuerying formulaQuerying = new FormulaQuerying(sessionData);
			...
			case STATE_RECEIVING_MODEL:
				...
				if (!ClientMessages.REQUEST_DIAGNOSIS_RESULT.equalsIgnoreCase(message)){ 					
					try {
						...
						ExquisiteAppXML appXML = StringPreprocessor.processForImport(xmlParser.getExquisiteAppXML(), this.sessionData.config);	
						...
						*/
						...
						this.sessionData.config.updateFromUserSettings(xmlParser.getExquisiteAppXML().getUserSettings());			
						response.status = StatusCodes.OK;
				...
				if (ClientMessages.REQUEST_DIAGNOSIS_RESULT.equalsIgnoreCase(message)){					
					if(this.sessionData.config.showServerDebugMessages){
						for (int i = 0; i < diagnoses.size(); i++){
							...
							System.out.println("    " + Utilities.printConstraintList(new ArrayList<Constraint>(diagnoses.get(i).getElements()), this.sessionData.diagnosisModel));
							System.out.println("--");
						}
	...
	private NotifyingThread initialiseDagEngineThread(){
		IDiagnosisEngine engine = EngineFactory.makeDAGEngineStandardQx(this.sessionData);
...
//		IDiagnosisEngine engine = EngineFactory.makeHeuristicSearchEngine(sessionData,4);
	...
	public void notifyOfThreadComplete(NotifyingThread thread){
		if (thread.isInterrupted()){
			...
			System.out.println("Found " + runner.diagnoses.size() + " diagnoses in " + (runner.endTime - runner.startTime) + " ms, shifting to tests.diagnosis ready state.");
			List<Diagnosis> runnerDiagnoses = runner.diagnoses;
			
			if (runnerDiagnoses.size()>0) {
				Diagnosis d = runnerDiagnoses.get(0);
				System.out.println("The conflicts: " + d.getElements().size());
				System.out.println(d.getElements());
			}
			
			
			diagnoses = new ArrayList<Diagnosis>(runnerDiagnoses);
