...
public class FormulaParserMZ {
	...
	public FormulaParserMZ(ExquisiteGraph<String> graph){
		this.graph = graph;
	}
	 ...
	 */
	public void buildChocoStatements(String id, String cellReference, Tree tree, Dictionary<String, IntegerExpressionVariable> variables, String worksheetReferencePrefix)
	{	
	 ...
	 * @return
	 */
	private Constraint buildConstraint(String cellReference, Tree tree, Dictionary<String, IntegerExpressionVariable> variables, String worksheetReferencePrefix)
	{			
//		System.out.println("-- " + tree.toString());
		...
		switch (tree.toString().toUpperCase())
        {
            ...
            case "FUNC":
            	return buildConstraint(cellReference, tree.getChild(0), variables, worksheetReferencePrefix);
            case "=":
            	return Choco.eq(buildExpression(cellReference, tree.getChild(0), variables, worksheetReferencePrefix), buildExpression(cellReference, tree.getChild(1), variables, worksheetReferencePrefix));
            case ">":
            	return Choco.gt(buildExpression(cellReference, tree.getChild(0), variables, worksheetReferencePrefix), buildExpression(cellReference, tree.getChild(1), variables, worksheetReferencePrefix));
            case "<":
            	return Choco.lt(buildExpression(cellReference, tree.getChild(0), variables, worksheetReferencePrefix), buildExpression(cellReference, tree.getChild(1), variables, worksheetReferencePrefix));
            case ">=":
            	return Choco.geq(buildExpression(cellReference, tree.getChild(0), variables, worksheetReferencePrefix), buildExpression(cellReference, tree.getChild(1), variables, worksheetReferencePrefix));
            case "<=":
            	return Choco.leq(buildExpression(cellReference, tree.getChild(0), variables, worksheetReferencePrefix), buildExpression(cellReference, tree.getChild(1), variables, worksheetReferencePrefix));
            case "<>":
            	return Choco.neq(buildExpression(cellReference, tree.getChild(0), variables, worksheetReferencePrefix), buildExpression(cellReference, tree.getChild(1), variables, worksheetReferencePrefix));
            default:
            	
            	if (tree.toString().equalsIgnoreCase(culture.AND())) {
            		return Choco.and(buildConstraintsFromChildren(cellReference, tree, variables, worksheetReferencePrefix));
            	}
            	else if (tree.toString().equalsIgnoreCase(culture.OR())) {
            		return Choco.or(buildConstraintsFromChildren(cellReference, tree, variables, worksheetReferencePrefix));
            	}
            	else if (tree.toString().equalsIgnoreCase(culture.NOT())) {
            		return Choco.not(buildConstraint(cellReference, tree.getChild(0), variables, worksheetReferencePrefix));
            	}
            	else
            	{
	 ...
	 * @return
	 */
	private Constraint[] buildConstraintsFromChildren(String cellReference, Tree tree, Dictionary<String, IntegerExpressionVariable> variables, String worksheetReferencePrefix)
	{
	 ...
	 * @return
	 */
	public IntegerExpressionVariable buildExpression(String cellReference, Tree tree, Dictionary<String, IntegerExpressionVariable> variables)
    {        
	 ...
	 * @return
	 */
	public IntegerExpressionVariable buildExpression(String cellReference, Tree tree, Dictionary<String, IntegerExpressionVariable> variables, String worksheetReferencePrefix)
    {
        if (tree == null)
            return null;
        ...
        else if (treeString.equalsIgnoreCase(culture.PLUS()))
        {
             ...
             {             	
            	...
            	if (isChild0Numeric && isChild1Numeric)
            	{
	            	...
	            	{
	            		intChild0 = IntegerUtilities.parseToInt(tree.getChild(0).toStringTree());
	            		return (Choco.plus(intChild0, buildExpression(cellReference, tree.getChild(1), variables, worksheetReferencePrefix)));   
	            	}
	             	if (isChild1Numeric)
		            {                    	
	             		intChild1 = IntegerUtilities.parseToInt(tree.getChild(1).toStringTree());
	              		return (Choco.plus(buildExpression(cellReference, tree.getChild(0), variables, worksheetReferencePrefix),intChild1));                    	
		            }
            	}            	
                return (Choco.plus(buildExpression(cellReference, tree.getChild(0), variables, worksheetReferencePrefix), buildExpression(cellReference, tree.getChild(1), variables, worksheetReferencePrefix)));
             }
             return null;
        }
        //"-"
        else if (treeString.equalsIgnoreCase(culture.MINUS()))
        {
        	if ((tree.getChild(0) != null) && (tree.getChild(1) != null))
            {
            	...
            	else
            	{
	            	...
	            	{
	            		intChild0 = IntegerUtilities.parseToInt(tree.getChild(0).toStringTree());
	            		return (Choco.minus(intChild0, buildExpression(cellReference, tree.getChild(1), variables, worksheetReferencePrefix)));   
	            	}
	            	if (isChild1Numeric)
	                {
	                	intChild1 = IntegerUtilities.parseToInt(tree.getChild(1).toStringTree());
	                	return (Choco.minus(buildExpression(cellReference, tree.getChild(0), variables, worksheetReferencePrefix),intChild1));
	                }
            	}
            	return (Choco.minus(buildExpression(cellReference, tree.getChild(0), variables, worksheetReferencePrefix), buildExpression(cellReference, tree.getChild(1), variables, worksheetReferencePrefix)));
            }
            return null;
        }
        //"*"
        else if (treeString.equalsIgnoreCase(culture.MULTIPLY()))
        {
        	if ((tree.getChild(0) != null) && (tree.getChild(1) != null))
            {
            	...
            	else
            	{
	            	...
	            	{
	            		intChild0 = IntegerUtilities.parseToInt(tree.getChild(0).toStringTree());
	            		return (Choco.mult(intChild0, buildExpression(cellReference, tree.getChild(1), variables, worksheetReferencePrefix)));   
	            	}
	        		if (isChild1Numeric)
	                {
	        			intChild1 = IntegerUtilities.parseToInt(tree.getChild(1).toStringTree());
	        			return (Choco.mult(buildExpression(cellReference, tree.getChild(0), variables, worksheetReferencePrefix),intChild1));
	                }
            	}
        		return (Choco.mult(buildExpression(cellReference, tree.getChild(0), variables, worksheetReferencePrefix), buildExpression(cellReference, tree.getChild(1), variables, worksheetReferencePrefix)));
            }
            return null;
        }
        //"/"
        else if (treeString.equalsIgnoreCase(culture.DIVIDE()))
        {
        	if ((tree.getChild(0) != null) && (tree.getChild(1) != null))
            {
            	...
            	else
            	{
	            	...
	            	{
	            		intChild0 = IntegerUtilities.parseToInt(tree.getChild(0).toStringTree());
	            		return (Choco.div(intChild0, buildExpression(cellReference, tree.getChild(1), variables, worksheetReferencePrefix)));   
	            	}
	        		if (isChild1Numeric)
	                {
	        			intChild1 = IntegerUtilities.parseToInt(tree.getChild(1).toStringTree());
	        			//System.out.println("Choco.mult(BuildExpression(" + tree.getChild(0) + ", {" + variables + "}, " + worksheetReferencePrefix + "), " + intChild1 + ")");
	        			return (Choco.div(buildExpression(cellReference, tree.getChild(0), variables, worksheetReferencePrefix),intChild1));
	                }
            	}
        		...
        		return (Choco.div(buildExpression(cellReference, tree.getChild(0), variables, worksheetReferencePrefix), buildExpression(cellReference, tree.getChild(1), variables, worksheetReferencePrefix)));
            }
            return null;
        }
        //Excel sum function
        ...
        else if (treeString.equalsIgnoreCase(culture.IF()))
        {
            ...
            {
        		return (Choco.ifThenElse(buildConstraint(cellReference, tree.getChild(0), variables, worksheetReferencePrefix), buildExpression(cellReference, tree.getChild(1), variables, worksheetReferencePrefix), buildExpression(cellReference, tree.getChild(2), variables, worksheetReferencePrefix)));
	 ...
	 * @return
	 */
	private IntegerExpressionVariable buildSumExpression(String cellReference, Tree tree, Dictionary<String, IntegerExpressionVariable> variables, String worksheetReferencePrefix) {
		if (tree == null)
            return null;
		...
		for ( int i = 0; i < childCount; i++ ) {
			...
			if(childString.contains(":")){
				...
				for (String cell : cells){
					...
					if (variable == null){
						if(isNumeric(cell)){
							IntegerVariable constant = constants.makeIntegerConstant("CONSTANT_", IntegerUtilities.parseToInt(cell));
